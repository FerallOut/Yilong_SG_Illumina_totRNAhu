...
```{r, load-count-data}
# # create the sample sheet for paired-end sequencing:
path_inDir <- "/data/manke/processing/balan/Yilong_SG_data/run_2218/2022.03_DE_genes_transcript"
counts_geneBody <- read.delim(paste0(path_inDir, "/1a_analysis_bam_default/featureCounts_geneBody/counts.tsv"), header = TRUE, sep='\t', na.strings="NA", row.names = 1, stringsAsFactors = F)
counts_exon <- read.delim(paste0(path_inDir, "/1a_analysis_bam_default/featureCounts_defaultExon/counts.tsv"), header = TRUE, sep='\t', na.strings="NA", row.names = 1, stringsAsFactors = F)
...
```{r, ploEISA-sgRNA}
# png(paste0(path_inDir, '/4_intron_analysis_bam/sgRNA_comparisons/','eisa_plot_sgRNA_LFC_revUV_redone2.png'), width=1600, height=800, units = "px", bg = "white")
# 
# par(oma = c(3,4,4,1),    # margins for whole plot
#     mar = c(3,4,3.2,1)  # margins for individual plots
#     )
...
## HAD TO CHANGE FROM VERSION 90 TO VERSION 91!!
...
path_orig <- "/data/manke/processing/balan/Yilong_SG_data/run_2218/2022.03_DE_genes_transcript/4_intron_analysis_bam"
...
## CREATED ALREADY WITH THE FOLLOWING
for (i in 1:length(list_toExtract) ) {
 ...
 png(paste0(path_inDir, '/4_intron_analysis_bam/sgRNA_comparisons/','eisa_plot_sgRNA_LFC_revUV2.png'), width=1600, height=800, units = "px", bg = "white")
