...
bcftools_call_01='SNP_TtoC_mutation_analysis/bcftools_call_mut01_bed'
...
#   ### Step C: run bcftools mpileup to call mutations on the extracted reads 
    bcftools mpileup  --threads 20 -d 10000 -Ou -R ${gene} -f ${ref} \
        ${main_dir}'/'${bcftools_extract_01}'/'$(basename ${k%%_*})'_reads_01NM.bam' |\
        bcftools call --threads 20 -mv -Ov --skip-variants indels \
        --pval-threshold 0.05 \
        --output ${main_dir}'/'${bcftools_call_01}'/'$(basename ${k%%_*})'_reads_01NM_bcftools_calls_bedR.vcf'
    t3=$(date +%s)
