...
out_dir=${main_dir}'/7b_bam_RG_merge_GATKcorr_HC_vcfs/1_forIGV_COMP2/4_IGV'
...
for k in $(find ${in_filt_dir} -iname '*_eq3.vcf.gz' | sort -V); do 
    ...
    zcat $k | awk '/#/ || ($4=="T" && $5=="C")' | bgzip -@ 20 > ${in_filt_dir}'/'$(basename ${k%%.vcf.gz})_TtoC.vcf.gz
    tabix -p vcf ${in_filt_dir}'/'$(basename ${k%%.vcf.gz})_TtoC.vcf.gz 
...
for k in $(find ${in_filt_dir} -iname '*_merge_eq3_TtoC.vcf.gz' | sort -V); do 
    echo $(basename ${k%%.vcf.gz})
