...
out_tsvs=${main_dir}'/7b_bam_RG_merge_GATKcorr_HC_vcfs'
in_bam='/data/manke/processing/balan/Yilong_SG_data/run_2218/data_int/bam'
out_txt=${out_tsvs}'/1_forIGV_COMP2/1_txt'
out_merge=${out_tsvs}'/1_forIGV_COMP2/2_merged'
dir_counts=${out_tsvs}'/1_forIGV_COMP2/3_counts'
...
do
    ...
    echo "s'$((3*$i+1))'_GATKCorr_SNPs.vcf.gz' s'$((3*$i+2))'_GATKCorr_SNPs.vcf.gz' s'$((3*$i+3))'_GATKCorr_SNPs.vcf.gz' "

    bcftools isec -Oz -p ${out_txt}'/'${arr_sample_names[$i]}'_gt2' -n+2 \
        ${out_tsvs}'/s'$((3*$i+1))'_GATKCorr_SNPs.vcf.gz' \
        ${out_tsvs}'/s'$((3*$i+2))'_GATKCorr_SNPs.vcf.gz' \
        ${out_tsvs}'/s'$((3*$i+3))'_GATKCorr_SNPs.vcf.gz' 
