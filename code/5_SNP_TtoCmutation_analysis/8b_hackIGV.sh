...
in_filt_dir=${main_dir}'/5_SNP_TtoCmutation_analysis/7b_bam_RG_merge_GATKcorr_HC_vcfs_QD2.0_FS60.0_MQ_40.0_HS_13.0_MQRS12.5'
out_dir=${main_dir}'/5_SNP_TtoCmutation_analysis/8b_bam_RG_merge_GATKcorr_HC_vcfs_QD2.0_FS60.0_MQ_40.0_HS_13.0_MQRS12.5_TtoC'

# retain only the T to C mutations (on col 4 and 5), filtering out all the others:
echo 'Retain only TtoC mutations'

for k in $(find ${in_filt_dir} -iname '*PASS.vcf.gz' | sort -V); do 
...
for k in $(find ${out_dir} -iname '*TtoC.vcf.gz' | sort -V); do 
    echo $(basename ${k%%.vcf.gz})
    ...
    paste <( zcat $k | awk -F '\t' '/^[^#]/{print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7}') \
        <( zcat $k | LC_NUMERIC=en_US.UTF-8 awk -F ":|," '/^[^#]/{print "AF="$7/($6+$7)}') \
        <( zcat $k | awk -F '\t' '/^[^#]/{print $9"\t"$10}') >> ${out_dir}'/'$(basename ${k%%.vcf.gz})_1AF.vcf
