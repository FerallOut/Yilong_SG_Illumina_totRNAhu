...
#############################################
    ...
    samtools view -h $k | grep 'XS:Z:Unassigned_NoFeatures' | cat header.sam - | samtools view -Sb - |  samtools sort -@ 4 -m 50G -o $(basename ${k%%.bam*})_NoFeatures_sort.bam; samtools index $(basename ${k%%.bam*})_NoFeatures_sort.bam
##########################################

    module load deeptools
# bamCoverage_raw:
    #bamCoverage -p 20 -b $(basename ${k%%.bam*})_NoFeatures_sort.bam -o $(basename ${k%%.bam*})_NoFeatures_sort_cov.bw

#bamCoverage_RPKM: NOT TESTED!!!
    bamCoverage -p 20 -b $(basename ${k%%.bam*})_NoFeatures_sort.bam  -o $(basename ${k%%.bam*})_NoFeatures_sort_cov.bw --binSize 25 --normalizeUsing RPKM --ignoreForNormalization ${ignoreForNormalization_list} --blackListFileName ${hg38_blacklist_bed}
