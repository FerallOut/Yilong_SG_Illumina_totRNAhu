...
corr_man='5_SNP_TtoCmutation_analysis/4a_bam_RG_merge_manCorr'
corr_GATK='5_SNP_TtoCmutation_analysis/4b_bam_RG_merge_GATKcorr'
#NM_man='5_SNP_TtoCmutation_analysis/5a_bam_RG_merge_manCorr_NM'
#NM_GATK='5_SNP_TtoCmutation_analysis/5b_bam_RG_merge_GATKcorr_NM'
#call_mutect2_manCorr='5_SNP_TtoCmutation_analysis/6a_bam_RG_merge_manCorr_NM_mutect2'
#call_mutect2_GATKcorr='5_SNP_TtoCmutation_analysis/6b_bam_RG_merge_GATKcorr_NM_mutect2'
call_HC_manCorr='5_SNP_TtoCmutation_analysis/6a_bam_RG_merge_manCorr_HaplotypeCaller'
call_HC_GATKcorr='5_SNP_TtoCmutation_analysis/6b_bam_RG_merge_GATKcorr_HaplotypeCaller'
ins_HC_GATKcorrERC='5_SNP_TtoCmutation_analysis/7b_bam_RG_merge_GATKcorr_HC_ERCvcfs'
ins_HC_GATKcorr='5_SNP_TtoCmutation_analysis/7b_bam_RG_merge_GATKcorr_HC_vcfs'

ins_HC_GATKcorrERC='5_SNP_TtoCmutation_analysis/7b_bam_RG_merge_GATKcorr_HC_ERCvcfs'
out_HC_GATKcorrERC='5_SNP_TtoCmutation_analysis/8b_GATKcorr_ERC_GenoGVCFs'
...
#
#     
####### <--- Section2: add RG groups with either samtools or picard ---> #######
#  
...
#
#    NO CHOICE MADE YET HERE
#    
#    #correct with samtools: change 255 to 60
#    t9=$(date +%s)
#    samtools view -h -@ 40 ${main_dir}'/'${merged_sam}'/'$(basename ${k%%.bam*})'_RG_sam.bam' |\
#         awk '{ FS=OFS="\t" } {if ($5 == '255') $5 = '60'; print}' |\
#         samtools view -@ 40 -Sb - > ${main_dir}'/'${corr_man}'/'$(basename ${k%%.bam*})'_RG_samCorr.bam' 
#
#    # DOES IT NEED SORTING?
#    samtools index -@ 20 ${main_dir}'/'${merged_sam}'/'$(basename ${k%%.bam*})'_RG_samCorr.bam'
#
#    t10=$(date +%s)
#    echo "** 255-60 manual correction for $(basename ${k%%.bam*}) took $(date -ud "@$(( $t10 - $t9 ))" +%T) (HH:MM:SS)"
#
#
...
#
#	
####### <--- Section 5: JUST FOR MUTECT2 add the NM tags to both samtools corrected and GATK corrected files, then index ---> #######
#
#    t13=$(date +%s)
#    echo "Adding NM tags, plus UQ and MD, to the files (otherwise Mutect2 errors out)"    
#
#    gatk SetNmMdAndUqTags -R ${ref} -I ${main_dir}'/'${corr_man}'/'$(basename ${k%%.bam*})'_RG_samCorr.bam' -O ${main_dir}'/'${NM_man}'/'$(basename ${k%%.bam*})'_RG_sam_NM.bam' 
#    samtools index -@ 20 ${main_dir}'/'${NM_man}'/'$(basename ${k%%.bam*})'_RG_sam_NM.bam' 
#
#    gatk SetNmMdAndUqTags -R ${ref} -I ${main_dir}'/'${corr_GATK}'/'$(basename ${k%%.bam*})'_RGsam_corrGATK.bam' -O ${main_dir}'/'${NM_GATK}'/'$(basename ${k%%.bam*})'_RG_GATK_NM.bam'
#    samtools index -@ 20 ${main_dir}'/'${NM_GATK}'/'$(basename ${k%%.bam*})'_RG_GATK_NM.bam'

#    t14=$(date +%s)
#    echo "** Adding NM tag for $(basename ${k%%.bam*}) took $(date -ud "@$(( $t14 - $t13 ))" +%T) (HH:MM:SS)"
#
#
...
#        -I ${main_dir}'/'${corr_man}'/'$(basename ${k%%.bam*})'_RG_samCorr.bam'  \
#        --bam-output ${main_dir}'/'${call_HC_manCorr}'/'$(basename ${k%%.bam*})'_RG_samCorrERC.bam' \
#        -ERC BP_RESOLUTION \
#        -O ${main_dir}'/'${call_HC_manCorr}'/'$(basename ${k%%.bam*})'_manCorrERC.vcf.gz'  2>${main_dir}'/'${call_HC_manCorr}'/log/'$(basename ${k%%.bam*})'_HaplotypeCallerERC.log' 
...
#    t16=$(date +%s)
#    echo "** Calling mutations with manually corrected HaplotypeCaller for ${file} took $(date -ud "@$(( $t16 - $t15 ))" +%T) (HH:MM:SS)"
#
#
#    t17=$(date +%s)
#
#    gatk HaplotypeCaller -R ${ref} -L ${gene_bed} \
#        -I ${main_dir}'/'${corr_man}'/'$(basename ${k%%.bam*})'_RG_samCorr.bam' \
#        --bam-output ${main_dir}'/'${call_HC_manCorr}'/'$(basename ${k%%.bam*})'_RG_samCorr.bam' \
#        -O ${main_dir}'/'${call_HC_manCorr}'/'$(basename ${k%%.bam*})'_manCorr.vcf.gz'  2>${main_dir}'/'${call_HC_manCorr}'/log/'$(basename ${k%%.bam*})'_HaplotypeCaller.log' 
#
#    gatk HaplotypeCaller -R ${ref} -L ${gene_bed} \
#        -I ${main_dir}'/'${corr_GATK}'/'$(basename ${k%%.bam*})'_corrGATK.bam' \
...
#
#    t18=$(date +%s)
#    echo "** Calling mutations with GATK corrected HaplotypeCaller for ${file} took $(date -ud "@$(( $t18 - $t17 ))" +%T) (HH:MM:SS)"
#
...
#        -O ${main_dir}'/'${out_HC_GATKcorrERC}'/''${file}'_ERC_genoGVCFs.vcf.gz --tmp-dir /data/extended/ \
...
#    gatk VariantFiltration -R ${ref} -V ${main_dir}'/'${ins_HC_manCorr}'/'$(basename ${k%%.bam*})'_manCorr_SNPs.vcf.gz'  \
#        -O ${main_dir}'/'${ins_HC_manCorr}'/'$(basename ${k%%.bam*})'_manCorr_SNPs_filt.vcf.gz' \
#        --filter-expression "QUAL<30.0 || QD < 2.0 || FS > 60.0 || MQ < 40.0 || SOR>3.0 || DP < 10" \
#	--filter-name "snp_filter" \
#        2>${main_dir}'/'${ins_HC_manCorr}'/log/'$(basename ${k%%.bam*})'_VarFilt.log'
#
#    gatk SelectVariants -R ${ref} -V ${main_dir}'/'${ins_HC_manCorr}'/'$(basename ${k%%.bam*})'_manCorr_SNPs_filt.vcf.gz' \
#         -O ${main_dir}'/'${ins_HC_manCorr}'/'$(basename ${k%%.bam*})'_manCorr_SNPs_filtPASS.vcf.gz'  --exclude-filtered \
#         2>${main_dir}'/'${ins_HC_manCorr}'/log/'$(basename ${k%%.bam*})'_PASSFilt.log'
#
#    t20=$(date +%s)
#    echo "** Filtering SNPs with manually correction for $(basename ${k%%.bam*}) took $(date -ud "@$(( $t20 - $t19 ))" +%T) (HH:MM:SS)"
#
    t21=$(date +%s)
...
#        -O ${main_dir}'/'${ins_HC_GATKcorr}'/'$(basename ${k%%.bam*})'_GATKCorr_SNPs_filt.vcf.gz' \
#        --filter-expression "QUAL<30.0 || QD < 2.0 || FS > 60.0 || MQ < 40.0 || SOR>3.0 || DP < 10" \
#	 --filter-name "snp_filter" \
#        2>${main_dir}'/'${ins_HC_GATKcorr}'/log/'$(basename ${k%%.bam*})'_VarFilt.log'
# 
#    gatk SelectVariants -R ${ref} -V ${main_dir}'/'${ins_HC_GATKcorr}'/'$(basename ${k%%.bam*})'_GATKCorr_SNPs_filt.vcf.gz' \
#         -O ${main_dir}'/'${ins_HC_GATKcorr}'/'$(basename ${k%%.bam*})'_GATKCorr_SNPs_filtPASS.vcf.gz'  --exclude-filtered \
#         2>${main_dir}'/'${ins_HC_GATKcorr}'/log/'$(basename ${k%%.bam*})'_PASSFilt.log'
